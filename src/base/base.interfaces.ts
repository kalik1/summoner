...
export type TReplaceBaseEntityWithStringsArrays<T> = {
  [K in keyof T]: [Extract<T[K], BaseEntity>] extends [never] ? T[K] : string;
};

export type EntityToDto<K> = TReplaceBaseEntityWithStringsArrays<ConstructorType<K>>;

export type EntityToDtoNoIdAndTimeStamps<K> = TReplaceBaseEntityWithStringsArrays<ConstructorType<K>> & {
  id: never
  createdAt?: never
  updatedAt?: never
};
