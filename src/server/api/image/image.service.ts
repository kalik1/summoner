...
export class ImageService {
  ...
  async create(createImageDto: CreateImageDto): Promise<ImageEntity | undefined> {
  ...
  findOne(id: string): Promise<ImageEntity | undefined> {
    return this.getOne(id);
  }

  async update(id: string, updateImageDto: UpdateImageDto): Promise<ImageEntity | undefined> {
    const newImage = await this.getOne(id);
    if (!newImage) throw new NotFoundException(`Image ${id} not found`);
