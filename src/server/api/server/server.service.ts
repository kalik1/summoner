...
export class ServerService {
  ...
  async create(createServerDto: CreateServerDto): Promise<ServerEntity | undefined> {
  ...
  async stop(id: ServerEntity['id']) {
    const currentServer: ServerEntity | undefined = await this.serverRepository.findOne(id, { relations: ['instance'] });
    if (!currentServer) throw new NotFoundException(`Server ${id} not found`);
    if (!currentServer.containterId) throw new NotFoundException(`Server ${id} has no container bound`);
  ...
  async start(id: ServerEntity['id']) {
    const currentServer: ServerEntity | undefined = await this.serverRepository.findOne(id, {
      relations: [
        'instance',
        'image',
        'image.imageType',
    ...
    if (!currentServer) throw new NotFoundException(`ServerEntity ${id} not found`);
  ...
  findOne(id: ServerEntity['id']): Promise<ServerEntity | undefined> {
  ...
  async update(id: ServerEntity['id'], updateServerDto: UpdateServerDto): Promise<ServerEntity | undefined> {
