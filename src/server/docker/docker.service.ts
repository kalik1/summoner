...
export class DockerService {
  ...
  private getInstanceFromName(InstanceName: string): Docker {
    const docker = this.dockers.find((d) => d.InstanceName === InstanceName);
    if (!docker) throw new NotFoundException(`docker ${InstanceName} not found`);
  ...
  GetOrStartClient(InstanceName: string, options: Docker.DockerOptions): DockerInstance {
    let dockerClient: DockerInstance | undefined = (this.dockers.find((d) => d.InstanceName === InstanceName));
    if (!dockerClient) throw new NotFoundException(`dockerClient ${InstanceName} not found`);
    if (!dockerClient) {
      dockerClient = {
        InstanceName,
  ...
  async CreateContainer(currentServer: ServerEntity): Promise<Docker.Container> {
    ...
    if (currentServer.managePort && containterConfig && containterConfig.ExposedPorts && containterConfig.HostConfig) {
